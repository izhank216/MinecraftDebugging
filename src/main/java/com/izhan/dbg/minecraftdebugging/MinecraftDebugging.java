...
public class MinecraftDebugging implements ClientModInitializer {
    ...
    public static class DebugOptionsScreen extends Screen {
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.drawText(textRenderer,"Logs:",10,30,0xffffff,false);

            int y = 50 - scroll;
            for(String log : LOGS) {
                context.drawText(textRenderer,log,10,y,0xffffff,false);
                y += 10;
            }

            int entityY = height/2;
            if(mc.world != null) {
                context.drawText(textRenderer,"Entities:",width-200,entityY,0xffff00,false);
                entityY += 15;
                for(Entity e : mc.world.getEntities()) {
                    context.drawText(textRenderer,e.getName().getString(),width-200,entityY,0xffffff,false);
                    entityY += 10;
            ...
            if(mc.crosshairTarget instanceof BlockHitResult hit) {
                ...
                context.drawText(textRenderer,"Block Inspector:",width-200,blockY,0x00ffff,false);
                blockY += 15;
                context.drawText(textRenderer,"Block: "+state.getBlock().getName().getString(),width-200,blockY,0xffffff,false);
                blockY += 10;
                context.drawText(textRenderer,"Pos: "+pos.toShortString(),width-200,blockY,0xffffff,false);
            }
